  ...
  return <div>
      ...
      <div className="moving">
        <form style={{ position: "relative" }} onSubmit={e => e.preventDefault()}>
          <input className="input" value={country1} onChange={handleChange} type="text" placeholder="Search a Country" />
          <Link className="linkButton" to={country1 !== "" ? "/main" : ""}>
            <button onClick={hanleInfoCount} className="button" type="submit">
              Search
            </button>
          </Link>
          {filteredSuggestions.length > 0 && <ul className={`suggestionsH ${displayingSugg ? "displaySugg" : ""}`}>
